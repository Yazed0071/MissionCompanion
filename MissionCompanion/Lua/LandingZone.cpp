...
std::string TrimStringLZ(const std::string& str) {
...
std::vector<std::string> SplitString(const std::string& str, char delimiter) {
    ...
    while (std::getline(stream, token, delimiter)) {
        token = TrimStringLZ(token);  // Remove leading/trailing spaces
        if (!token.empty()) {
...
std::string GetListLandingZone(const std::string& checkedLZs) {
    ...
    Logger::Info("Checked Landing Zones:");
    for (const auto& zone : checkedLZsVector) {
        Logger::Info(" - %s", zone.c_str());
    }

    bool firstEntry = true; 

    // Iterate over predefined landing zones and check matches
    for (const auto& lz : landingZones) {
        for (const auto& checkedLZ : checkedLZsVector) {
            if (lz.find(checkedLZ) != std::string::npos) {
                Logger::Info("Match Found: %s", checkedLZ.c_str());
    ...
    Logger::Info("Final HeliLandPoint Output:\n%s", result.str().c_str());
    return result.str();
}
